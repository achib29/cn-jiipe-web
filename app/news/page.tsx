...
import { ChevronLeft, ChevronRight, Flame, ArrowRight, Calendar } from 'lucide-react'; 
...
interface Article {
  ...
  date: string;
  is_hot?: boolean;
  hot_priority?: number | null; // <--- tambahkan ini
}
...
export default function NewsIndexPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Cast agar dikenali sebagai Article[]
        ...
        // 2. LOGIKA FEATURED BERDASARKAN hot_priority (manual)
        //    1 = Main Story (besar kiri), 2 & 3 = Featured kanan
        const mainStory   = allArticles.find(a => a.is_hot && a.hot_priority === 1);
        const featured2   = allArticles.find(a => a.is_hot && a.hot_priority === 2);
        const featured3   = allArticles.find(a => a.is_hot && a.hot_priority === 3);
        ...
        const sliderArticles = allArticles.filter(a => !featuredIds.includes(a.id));
  ...
  // Navigasi halaman (dipakai untuk tombol panah & pagination angka)
  const nextPage = () => {
    setCurrentPage((prev) => (prev < totalPages ? prev + 1 : 1)); // loop ke halaman 1
  };

  const prevPage = () => {
    setCurrentPage((prev) => (prev > 1 ? prev - 1 : totalPages)); // loop ke halaman terakhir
  };
  ...
  const MainFeaturedCard = ({ article }: { article: Article }) => (
    <Link href={`/news/${article.slug}`} className="group block h-full w-full relative rounded-[1.5rem] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-200/50">
        <img 
  ...
  const SubFeaturedCard = ({ article }: { article: Article }) => (
    <Link href={`/news/${article.slug}`} className="group block h-full w-full relative rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-500 border border-gray-200/50">
        <img 
            src={article.coverImage} 
            alt={article.title} 
            className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
        ...
        <div className="absolute bottom-0 left-0 p-5 w-full z-20">
            ...
            <div className="flex items-center justify-between">
                ...
                <ArrowRight size={14} className="text-orange-400 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-[-5px] group-hover:translate-x-0"/>
            </div>
        </div>
    </Link>
  );
  ...
  const SliderCard = ({ article }: { article: Article }) => (
    <Link href={`/news/${article.slug}`} className="group block h-[420px] relative rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-500 border border-gray-200 bg-white">
  ...
  return (
    <main className="min-h-screen bg-gray-50 pt-20">
      ...
      <section className="container mx-auto px-4 py-12 max-w-7xl">
        ...
        {/* --- 1. FEATURED STORIES (DYNAMIC LAYOUT) --- */}
        ...
        {/* --- 2. LATEST UPDATES (PAGINATION SYSTEM) --- */}
