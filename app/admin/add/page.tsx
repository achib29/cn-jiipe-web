...
import { 
  ...
  ImagePlus, Loader2, ArrowLeft, Save, Flame, Calendar, CheckCircle, AlertCircle
} from "lucide-react";

// --- KOMPONEN MODAL / POPUP NOTIFIKASI ---
// Kita buat komponen ini di dalam file yang sama agar mudah di-copy paste
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center transform transition-all animate-in zoom-in-95 duration-300 border border-gray-100">
        ...
        {/* Text Content */}
        <h3 className={`text-2xl font-bold mb-2 ${isSuccess ? 'text-gray-900' : 'text-red-600'}`}>
          {title}
        </h3>
        ...
        <button 
          onClick={onClose}
          className={`w-full py-3.5 rounded-xl text-white font-bold shadow-lg transition-transform transform hover:scale-[1.02] active:scale-[0.98] ${
            isSuccess 
...
export default function AddNewsPage() {
  ...
  const [hotPriority, setHotPriority] = useState<number | null>(null);
      ...
      const fetchArticle = async () => {
        ...
        if (error) {
          ...
          setModal({
             ...
             title: 'Gagal Memuat Data', message: 'Tidak dapat mengambil data artikel dari server.' 
          });
        } else if (data) {
          setTitle(data.title);
  ...
  const handleCoverImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      if (file.size > 2 * 1024 * 1024) { 
          setModal({ isOpen: true, type: 'error', title: 'File Terlalu Besar', message: 'Ukuran gambar maksimal 2MB.' });
          return;
      }
      setImageFile(file);
  ...
  const handleBodyImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setModal({ isOpen: true, type: 'error', title: 'Upload Gagal', message: 'Gagal mengupload gambar ke dalam artikel.' });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !content) {
        setModal({ isOpen: true, type: 'error', title: 'Data Belum Lengkap', message: 'Judul dan Konten artikel wajib diisi.' });
        return;
    }
    ...
    try {
      ...
      const articleData: any = {
        ...
        status,
        is_hot: isHot, 
        hot_priority: isHot ? hotPriority : null, 
        summary,
      ...
      if (isHot && hotPriority) {
        if (editId) {
          await supabase.from('articles').update({ is_hot: false, hot_priority: null }).eq('hot_priority', hotPriority).neq('id', editId);
        } else {
          await supabase.from('articles').update({ is_hot: false, hot_priority: null }).eq('hot_priority', hotPriority);
        }
      }
      ...
      setModal({
        ...
        message: editId ? 'Artikel Anda telah berhasil diperbarui.' : 'Artikel baru telah berhasil diterbitkan ke website.'
      });

    } catch (error: any) {
      console.error("Error:", error);
      setModal({ isOpen: true, type: 'error', title: 'Gagal Menyimpan', message: error.message });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-gray-500">{editId ? "Update existing content" : "Create content for JIIPE Website"}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 h-[calc(100vh-200px)]">
          ...
          <div className="space-y-6 h-full overflow-y-auto pb-10">
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
              ...
              {isHot && (
                <div className="mt-4">
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Main / Featured Order</label>
                  <select value={hotPriority ?? ''} onChange={(e) => { const val = e.target.value; setHotPriority(val ? parseInt(val, 10) : null); }} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-red-500 text-sm">
                  ...
                  <p className="text-[11px] text-gray-400 mt-1">Urutan ini dipakai untuk menentukan Main Story & Featured di halaman depan.</p>
                </div>
              )}
            </div>
            ...
            {/* PUBLISH DATE */}
            <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
              <label className="block text-xs font-bold text-gray-500 uppercase mb-3 flex items-center gap-2"><Calendar size={14} /> Publish Date</label>
              <input type="date" required value={publishDate} onChange={(e) => setPublishDate(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-red-500 font-bold text-gray-700"/>
